class ChartsController < ApplicationController
    ...
    def admin_group_stats
        group_data = Statistic.order(year: :desc).group(:year).group('20 - weeks_to_syg').sum(:number_of_groups).chart_json

        render json: group_data
    end

    def admin_participant_stats
        participant_data = Statistic.order(year: :desc).group(:year).group('20 - weeks_to_syg').sum(:number_of_participants).chart_json

        render json: participant_data
    end

    def admin_sport_entry_stats
        sport_data = Statistic.order(year: :desc).group(:year).group('20 - weeks_to_syg').sum(:number_of_sport_entries).chart_json

        render json: sport_data
    end

    def admin_volunteer_stats
        volunteer_data = Statistic.order(year: :desc).group(:year).group('20 - weeks_to_syg').sum(:number_of_volunteer_vacancies).chart_json
    ...
    def evening_opening_preferences
        @total_groups = Group.coming.not_admin.count
    
        early = Group.coming.not_admin.fri_early_service.count
        late = Group.coming.not_admin.fri_late_service.count
