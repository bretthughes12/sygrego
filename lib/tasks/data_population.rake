namespace :syg do
  ...
  desc 'Convert Groups Grades to Groups Sports filters'
  task convert_groups_grades_to_sports: ['db:migrate'] do |t|
    puts 'Converting groups grades to sports...'

    count = 0
    errors = 0
    ignored = 0

    GroupsGradesFilter.all.each do |f|
      group = f.group
      ms = group.mysyg_setting
      sport = f.grade.sport

      if ms.team_sport_view_strategy == 'Show listed' || ms.indiv_sport_view_strategy == 'Show listed'
        n = GroupsSportsFilter.new(group_id: group.id, sport_id: sport.id)
